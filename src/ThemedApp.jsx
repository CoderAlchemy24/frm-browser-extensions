...
const ThemedApp = () => {
  ...
  useEffect(() => {
    (async function fetchData() {
      try {
        ...
        // Normalizálás: legyen minden elemnek isActive property-je
        setCards(data.map((c, i) => ({
          ...c,
          id: (c.id !== undefined && c.id !== null) ? c.id : i,
          isActive: (c.isActive !== undefined && c.isActive !== null) ? c.isActive : ((c.active !== undefined && c.active !== null) ? c.active : false),
        })));
      } catch (e) {
        console.error(e);
      }
  ...
  // Egy kártya kapcsolójának kezelése
  const handleToggleCard = useCallback((id, nextValue) => {
    setCards(cards =>
      cards.map(card =>
        card.id === id ? { ...card, isActive: nextValue } : card
      )
    );
  }, []);
    ...
    <main className={
      ...
      <section className={clsx(
        ...
        <div className={clsx('flex flex-row gap-2.5 mr-4')}>
          ...
          <button
            className={clsx(
              'm-1 px-4 py-2 text-lg h-[46px] w-[95px] rounded-full transition-colors duration-200 cursor-pointer  focus:outline-sky-950',
              filter === 'active'
                ? 'bg-[#C7231A] text-white'
                : 'bg-gray-300 text-gray-700',
              'border-none outline-none focus:ring-2 focus:ring-[#C7231A]'
          ...
          <button
            className={clsx(
              'm-1 px-4 py-2 text-lg h-[46px] w-[112px] rounded-full transition-colors duration-200 cursor-pointer  focus:outline-sky-950',
              filter === 'inactive'
                ? 'bg-[#C7231A] text-white'
                : 'bg-gray-300 text-gray-700',
              'border-none outline-none focus:ring-2 focus:ring-[#C7231A]'
        ...
        <div className="flex flex-row justify-center gap-2 flex-wrap m-25">
